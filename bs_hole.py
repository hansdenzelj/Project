...
empty = np.zeros(mask.shape, dtype=np.uint8) #black
coloredMask = np.invert(mask) #invert 0's and 1's -> 0, 255
coloredMask = cv.merge((empty, coloredMask, empty)) #0, 255, 0

#print(coloredMask[1]) #stores data on the green background
...
while True:
    ...
    if results[0].masks is not None:
        ...
        # p_indices = np.where(mask_data == 255) #person detected, 255
